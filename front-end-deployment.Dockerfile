...
##
## despite this image only needing the front ends built, to make use of the yarn.lock file for the
## monorepo, we need all the package.json files to be present
...
## build internal dependencies of front ends
COPY packages/utils/. ./packages/utils
RUN yarn workspace @tupaia/utils build
COPY packages/ui-components/. ./packages/ui-components
RUN yarn workspace @tupaia/ui-components build

## copy in all front end src code, ready to be built by codeship-steps (won't build here because
## env vars are needed)
COPY packages/admin-panel/. ./packages/admin-panel
COPY packages/lesmis/. ./packages/lesmis
