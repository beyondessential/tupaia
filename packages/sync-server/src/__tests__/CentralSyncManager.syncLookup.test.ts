...
import { SyncLookupQueryDetails } from '../types';
...
describe('Sync Lookup data', () => {
  // Increase timeout for this test suite due to heavy database operations in beforeAll
  ...
  it('Snapshots project linked records when it is a newly selected project', async () => {
    ...
    for (const model of outgoingModels) {
      ...
      // const data = await syncLookupRecord.getData();
