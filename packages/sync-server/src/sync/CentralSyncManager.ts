  ...
  async updateLookupTable() {
    ...
    try {
      ...
      await this.database.wrapInTransaction(async (database: TupaiaDatabase) => {
        ...
        // Otherwise, update it to SYNC_LOOKUP_PENDING_UPDATE_SYNC_TICK so that
        ...
        const syncLookupTick = isInitialBuildOfLookupTable ? null : SYNC_LOOKUP_PENDING_UPDATE_SYNC_TICK;
      ...
      // 1. When starting updating lookup table, use fixed -1 as the tick and treat them as pending updates (SYNC_LOOKUP_PENDING_UPDATE_SYNC_TICK)
