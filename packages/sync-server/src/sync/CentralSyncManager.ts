...
export class CentralSyncManager {
  ...
  async checkSessionReady(sessionId: string): Promise<boolean | void> {
  ...
  async checkPullReady(sessionId: string): Promise<boolean | void> {
    await this.connectToSession(sessionId);
  ...
  async setupSnapshotForPull(
    ...
    try {
      ...
      await this.database.wrapInTransaction(async (database: TupaiaDatabase) => {
        ...
        // await removeEchoedChanges(this.store, sessionId); // TODO: Add this
      });
