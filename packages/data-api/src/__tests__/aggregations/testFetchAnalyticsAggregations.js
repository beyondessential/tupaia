...
export const testFetchAnalyticsAggregations = assertCorrectResponse => () => {
  ...
  describe('COUNT_PER_ORG_GROUP', () => {
    it('should return correct results across a number of data_elements, entities, and periods', async () => {
      ...
      const countAnswers = answerCode =>
        cropResponses.reduce((count, { answers }) => count + (answers[answerCode] ? 1 : 0), 0);

      const crop1CountedResponse = {
      ...
      await assertCorrectResponse(
        {
          ...
          aggregations: [
            { type: 'COUNT_PER_ORG_GROUP', config: { orgUnitMap: CITY_TO_COUNTRY_MAP } },
          ],
        },
        [
          setResponseEntityCode(crop1CountedResponse, 'NZ'),
          setResponseEntityCode(crop2CountedResponse, 'NZ'),
        ],
      );
    });
  });

  describe('COUNT_PER_PERIOD_PER_ORG_GROUP', () => {
    it('should return correct results across a number of data_elements, entities, and periods', async () => {
      const countAnswers = (answerCode, responses) =>
        responses.reduce((count, { answers }) => count + (answers[answerCode] ? 1 : 0), 0);

      const makeCountedResponse = responsesFromSameDate => {
        const countedResponse = {
          ...responsesFromSameDate[0],
          answers: {
            KITTY_1: countAnswers('KITTY_1', responsesFromSameDate),
            KITTY_2: countAnswers('KITTY_2', responsesFromSameDate),
          },
        };
        return setResponseEntityCode(countedResponse, 'NZ');
      };

      const kitty20210104CountedResponse = makeCountedResponse([
        KITTY_RESPONSE_WELLINGTON_MORNING_20210104W1,
        KITTY_RESPONSE_WELLINGTON_MIDDAY_20210104W1,
        KITTY_RESPONSE_WELLINGTON_NIGHT_20210104W1,
      ]);

      const kitty20210105CountedResponse = makeCountedResponse([
        KITTY_RESPONSE_WELLINGTON_MORNING_20210105W1,
        KITTY_RESPONSE_WELLINGTON_NIGHT_20210105W1,
      ]);

      const kitty20210113CountedResponse = makeCountedResponse([
        KITTY_RESPONSE_WELLINGTON_MORNING_20210113W2,
      ]);
      const kitty20210115CountedResponse = makeCountedResponse([
        KITTY_RESPONSE_WELLINGTON_MORNING_20210115W2,
      ]);
      const kitty20210205CountedResponse = makeCountedResponse([
        KITTY_RESPONSE_WELLINGTON_MORNING_20210205W5,
      ]);
      const kitty20210608CountedResponse = makeCountedResponse([
        KITTY_RESPONSE_AUCKLAND_MORNING_20210608W23,
      ]);
      const kitty20210219CountedResponse = makeCountedResponse([
        KITTY_RESPONSE_WELLINGTON_MORNING_20210219W7,
      ]);
      const kitty20220606CountedResponse = makeCountedResponse([
        KITTY_RESPONSE_WELLINGTON_MORNING_20220606W23,
      ]);
      const kitty20220608CountedResponse = makeCountedResponse([
        KITTY_RESPONSE_WELLINGTON_MORNING_20220608W23,
        KITTY_RESPONSE_AUCKLAND_MORNING_20220608W23,
      ]);
      ...
      await assertCorrectResponse(
        {
          ...
          dataElementCodes: ['KITTY_1', 'KITTY_2'],
          aggregations: [
            { type: 'COUNT_PER_PERIOD_PER_ORG_GROUP', config: { orgUnitMap: CITY_TO_COUNTRY_MAP } },
          ],
        },
        [
          kitty20210104CountedResponse,
          kitty20210105CountedResponse,
          kitty20210113CountedResponse,
          kitty20210115CountedResponse,
          kitty20210205CountedResponse,
          kitty20210219CountedResponse,
          kitty20210608CountedResponse,
          kitty20220606CountedResponse,
          kitty20220608CountedResponse,
        ],
      );
    });
  });
