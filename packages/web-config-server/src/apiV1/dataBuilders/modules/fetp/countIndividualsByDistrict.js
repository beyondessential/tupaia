...
export const countIndividualsByDistrict = async ({ models, dataBuilderConfig, entity, fetchHierarchyId }) => {
    ...
    const subDistricts = await entity.getDescendantsOfType(hierarchyId, models.entity.types.SUB_DISTRICT);
    const individuals = await entity.getDescendantsOfType(hierarchyId, models.entity.types.INDIVIDUAL);
    ...
    const rows = districts.sort(compare).map(district => {
    ...
    const returnData = { 
        columns: [
            {key: 'CountColumn', title: dataBuilderConfig.columns[0] }
        ], 
        rows,
    };
    return returnData;
