...
import { getSortByKey, utcMoment, reduceToDictionary, stripFromEnds } from '@tupaia/utils';
...
class TableOfEventsBuilder extends DataBuilder {
  ...
  buildColumns() {
    const getDefaultTitle = key =>
      stripFromEnds(this.dataSources[key].name, this.config.stripFromColumnNames);
