...
import { LightTab } from '@tupaia/ui-components';
...
const TabLink = props => (
  <LightTab {...props} isActive={isActive} activeClassName="Mui-selected" component={NavLink} />
);
      ...
      <Inner>
        <NavLinks>
          ...
          {links.map(({ label, to, icon }) => (
            <StyledTabLink key={to} to={to} value={to}>
              {icon}
              {label}
            </StyledTabLink>
          ))}
