...
import { Resolved, UserAccount } from '@tupaia/types';
...
export class AuthRoute extends Route<AuthRequest> {
  ...
  private async userPasswordLogin(
    ...
    const [user]: UserAccount[] = await this.req.ctx.services.central.fetchResources('user', {
      filter: { email: userEmail },
      columns: ['email', 'id'],
    });

    if (!user) {
      throw new Error(`Could not find user for email: ${userEmail}`);
    }
