...
describe('countDistinct', () => {
  const testData: [string, { array: any[]; mapper?: ((x: any) => any) | string }, number][] = [
    ['countDistinct', { array: [] }, 0],
    ['single item ', { array: [0] }, 1],
    ['multiple items, same value', { array: [0, 0] }, 1],
    ['multiple items, different values', { array: [0, 1] }, 2],
    ['multiple items, mixed same & different values', { array: [0, 1, 0, 1, 1, 2] }, 3],
    ['function mapper', { array: [1.1, 2, 1.3], mapper: Math.floor }, 2],
    [
      'string mapper',
      {
        array: [
  ...
  it('throws on invalid string mapper', () => {
    expect(() => countDistinct([{ age: 1 }, { age: 2 }, 7], 'age')).toThrowError(
...
describe('removeAt', () => {
  describe('removes the item at the specified index', () => {
    const testData: [string, [any[], number], any[]][] = [
      ['empty array', [[], 0], []],
