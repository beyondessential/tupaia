...
- Android SDK
  - Look for 'get just the command line tools' at the bottom of [the Android Studio Download page](https://developer.android.com/studio/index.html)
    - Note: since new updates from android, after unzipping the `command line tools` package, you'll get a folder named `cmdline-tools`. To make it works, rename the unpacked directory from `cmdline-tools` to `tools`, and place it under `~/android-sdk/cmdline-tools/`. Now it should look like `~/android-sdk/cmdline-tools/tools/bin/`
