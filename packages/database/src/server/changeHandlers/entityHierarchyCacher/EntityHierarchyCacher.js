 ...
 *
 * TUP-3068 simplification: this used to walk a 3-class pipeline (Cacher →
 * SubtreeRebuilder → ParentChildBuilder) and conditionally pick between
 * `entity_relation` and `entity.parent_id` at each level. After RN-1853 and TUP-3065,
 * `entity.parent_id` is the source of truth for direct edges and `project_country` is
 * the bridge between projects and countries. The rebuild is now a single recursive
 * CTE per project — see ClosureCacheBuilder.
 *
 * Triggered by:
...
export class EntityHierarchyCacher extends ChangeHandler {
  ...
  async translateEntityChange({ type, old_record: oldRecord, new_record: newRecord }) {
    ...
    const candidateEntityIds = [oldRecord?.id, newRecord?.id, oldRecord?.parent_id, newRecord?.parent_id].filter(Boolean);
  ...
  async handleChanges(transactingModels, projectIds) {
    const builder = new ClosureCacheBuilder(transactingModels);
