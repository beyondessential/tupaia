    ...
    config: {
      ...
      transform: [
        ...
        'mostRecentValuePerOrgUnit',
        {
          ...
          "'NumberOfSchools'": '1',
          "'DevelopmentPartners'":
            '{ AEAL: exists($row.SchDP_AEAL), CRS: exists($row.SchDP_CRS), HII: exists($row.SchDP_HII), Plan: exists($row.SchDP_Plan), RtR: exists($row.SchDP_RtR), UNICEF: exists($row.SchDP_UNICEF), WB: exists($row.SchDP_WB), WC: exists($row.SchDP_WC), WFP: exists($row.SchDP_WFP), WR: exists($row.SchDP_WR), WV: exists($row.SchDP_WV), Other: exists($row.SchCVD023) }',
        },
        {
          transform: 'aggregate',
          NumberOfStudents: 'sum',
          NumberOfSchools: 'count',
          DevelopmentPartners: 'max',
        },
      ],
    },
    permission_group_id: await permissionGroupNameToId(db, 'Public'),
