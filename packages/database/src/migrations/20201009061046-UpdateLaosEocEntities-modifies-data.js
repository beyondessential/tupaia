...
const PROVINCES = {
  ...
  'Vientiane Capital': {
    capital: {
      name: 'Sisattanak',
      lat: 17.93219235318203,
      lon: 102.63719558715822,
    },
  },
  'Vientiane Province': {
    capital: {
...
const clearData = async function (db) {
  await db.runSql(
    "DELETE FROM entity_relation where child_id IN (SELECT id FROM entity WHERE code like 'LA_PROV_CAPITAL_%');",
  );
  await db.runSql("DELETE FROM entity WHERE code like 'LA_PROV_CAPITAL_%';");
};
