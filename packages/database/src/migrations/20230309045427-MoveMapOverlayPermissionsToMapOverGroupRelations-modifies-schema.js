...
exports.up = async function (db) {
  ...
  // Step 3: Convert permission_group column to permission_group_ids
  await db.runSql(`
    ALTER TABLE map_overlay ADD COLUMN permission_group_ids TEXT[] DEFAULT '{}';
  `);
  await db.runSql(`
    UPDATE map_overlay mo
    SET permission_group_ids = permission_group_ids || (SELECT id FROM permission_group WHERE name = mo.permission_group LIMIT 1);
  `);
  await db.runSql(`
...
exports.down = async function (db) {
  // Step 1: Convert permission_group_ids column to permission_group
  await db.runSql(`
    ALTER TABLE map_overlay ADD COLUMN permission_group TEXT NOT NULL;
  `);
  await db.runSql(`
    UPDATE map_overlay mo
    SET permission_group = (SELECT name FROM permission_group WHERE id = mo.permission_group_ids[0] LIMIT 1);
  `);
  await db.runSql(`
    ALTER TABLE map_overlay DROP COLUMN permission_group_ids;
  `);
