...
exports.up = async function (db) {
  ...
  // so drop it.
  await db.runSql(
    'CREATE INDEX IF NOT EXISTS survey_response_user_id_data_time_idx ON survey_response (user_id, data_time DESC);',
  );
  await db.runSql('DROP INDEX IF EXISTS survey_response_user_id_idx;');
  ...
  await db.runSql(
    'CREATE INDEX IF NOT EXISTS task_assignee_id_status_due_date_idx ON task (assignee_id, status, due_date);',
  );
  await db.runSql(
    'CREATE INDEX IF NOT EXISTS task_comment_task_id_type_idx ON task_comment (task_id, type);',
  );
  ...
  // relation).
  await db.runSql('DROP INDEX IF EXISTS task_initial_request_id_fk;');
  await db.runSql(
...
exports.down = async function (db) {
  await db.runSql('DROP INDEX IF EXISTS survey_response_user_id_data_time_idx;');
  await db.runSql(
    'CREATE INDEX IF NOT EXISTS survey_response_user_id_idx ON survey_response (user_id);',
  );

  await db.runSql('DROP INDEX IF EXISTS task_assignee_id_status_due_date_idx;');
  await db.runSql('DROP INDEX IF EXISTS task_comment_task_id_type_idx;');

  await db.runSql('DROP INDEX IF EXISTS task_initial_request_id_idx;');
  await db.runSql(
