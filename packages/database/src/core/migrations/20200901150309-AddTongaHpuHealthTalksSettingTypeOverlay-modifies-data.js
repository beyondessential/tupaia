...
const COLOUR_MAP_OVERLAY = {
  id: 'TO_HPU_Health_Talks_Training_Type_Setting_Unique',
...
const TOTAL_MAP_OVERLAY = {
  id: 'TO_HPU_Health_Talks_Training_Type_Setting_ZZZ_Total',
  name: 'Total Sessions',
  userGroup: 'Tonga Health Promotion Unit',
  dataElementCode: 'value',
  isDataRegional: false,
...
exports.up = async function (db) {
  ...
  for (const dataValue of DATA_VALUES) {
    TOTAL_MAP_OVERLAY.id = `TO_HPU_Health_Talks_Training_Type_Setting_Type_${dataValue}`;
    TOTAL_MAP_OVERLAY.name = `${dataValue} Sessions`;
    TOTAL_MAP_OVERLAY.measureBuilderConfig.dataValues.HP281 = {
      value: dataValue,
      operator: '=',
    };
...
exports.down = function (db) {
  return db.runSql(`
      ...
      WHERE id LIKE '%TO_HPU_Health_Talks_Training_Type_Setting_%';
  `);
};
