...
const NUMBER_OF_STUDENTS_REPORT_CODES = [
  'LESMIS_ESSDP_ece_NumberOfStudents',
  'LESMIS_ESSDP_lse_NumberOfStudents',
  'LESMIS_ESSDP_use_NumberOfStudents',
  'LESMIS_ESSDP_LSE_HLO1',
  'LESMIS_ESSDP_USE_HLO1',
  'LESMIS_ESSDP_ECE_HLO1',
];

// Taken wholesale from ConveryExistingReportsToNewTransformSyntax
...
// Fetch only the reports that have been updated on this branch
const getReports = async db =>
  (
    await db.runSql(
      `SELECT * from report WHERE code IN (${arrayToDbString(NUMBER_OF_STUDENTS_REPORT_CODES)})`,
    )
  ).rows;
