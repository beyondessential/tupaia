...
export class EntityModel extends MaterializedViewLogDatabaseModel {
  ...
  async getDescendantsFromParentChildRelation(hierarchyId, entityIds, params = {}) {
    ...
    return this.runCachedFunction(cacheKey, async () => {
      return this._getDescendantsRecursively(hierarchyId, entityIds, params);
    });
  }
  ...
  async getAncestorsFromParentChildRelation(hierarchyId, entityIds, params = {}) {
    ...
    return this.runCachedFunction(cacheKey, async () => {
      return this._getAncestorsRecursively(hierarchyId, entityIds, params);
    });
  }
   ...
   * @returns {Promise<EntityRecord[]>} Map of entity ID to entity data
   */
  async _getDescendantsRecursively(
    hierarchyId,
    parentIds,
    params = {},
    allDescendants = [],
   ...
   */
  async _getAncestorsRecursively(hierarchyId, childIds, params = {}, allAncestors = [], level = 0) {
    const { filter = {}, pageSize } = params;
    ...
    return this._getAncestorsRecursively(
      hierarchyId,
      nextLevelChildIds,
      params,
      newAllAncestors,
