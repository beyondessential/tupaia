...
export class DataServiceResolver {
   ...
   */
  async resolveDataElements(dataElements, entity = {}) {
    const { country_code: countryCode } = entity;
    ...
    for (const dataElement of dataElements) {
      ...
      if (deMappings.length > 1) {
        throw new Error(`Multiple mappings found for Data Element ${dataElement.code}`);
      }
