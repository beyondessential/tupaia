...
export function addRoutesToApp(app: Express) {
   ...
   * POST routes
   */
  app.post('(/v[0-9]+)?/testFetchReport', wrappedFetchReportWithAsyncCatch); // POST endpoint for testing fetching a report (able to provide test data and config in request body)
