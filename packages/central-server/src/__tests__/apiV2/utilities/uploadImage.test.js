...
const mockS3Client = createJestMockInstance('@tupaia/server-utils', 'S3Client', {
  uploadImage: jest.fn(async () => EXAMPLE_UPLOADED_IMAGE_URL),
});
const mockS3 = createJestMockInstance('@tupaia/server-utils', 'S3');
...
describe('uploadImage', () => {
  ...
  it('uploads the image and returns the result if is a base64 encoded image', async () => {
    ...
    expect(mockS3Client.deleteFile).not.toHaveBeenCalled();
  });
  ...
  it('removes the existing image if path is passed in', async () => {
    ...
    expect(mockS3Client.deleteFile).toHaveBeenCalledOnceWith('existingImageUrl');
  });
