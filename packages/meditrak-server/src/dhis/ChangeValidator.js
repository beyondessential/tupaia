...
export class ChangeValidator {
  ...
  validate = async changes => {
    console.log('validating', changes.length);
    const validations = changes.map(async change => {
      ...
      if (change.type === 'delete') {
        ...
        const existingSyncLogRecord = await this.models.dhisSyncLog.findOne({
          record_id: recordId,
        });
      ...
      if (recordType === this.models.answer.databaseType) {
        ...
        surveyResponse = await this.models.surveyResponse.findOne({
          id: answer.survey_response_id,
        });
