...
/**
 * Responds to GET requests to the /export/surveyResponses endpoint, exporting excel documents
 * containing the relevant survey responses
 */
export class SurveyResponseExporter {
  constructor(models, userId, accessPolicy) {
    this.models = models;
    this.userId = userId;
  ...
  async export({
    ...
    try {
      ...
      for (let surveyIndex = 0; surveyIndex < surveys.length; surveyIndex++) {
        ...
        if (!hasBESAdminAccess(this.accessPolicy)) {
          if (entities.length > 0) {
            ...
            entities.forEach(entity => {
              hasSurveyAccess =
                hasSurveyAccess &&
                this.accessPolicy.allows(entity.country_code, permissionGroup.name);
            });
          } else {
            // Check we have access to the singular country
            hasSurveyAccess = this.accessPolicy.allows(country.code, permissionGroup.name);
          }
        }
        if (!hasSurveyAccess) {
        ...
        if (surveyResponse) {
          const entity = await this.models.entity.findById(surveyResponse.entity_id);
          const answers = await findAnswersInSurveyResponse(this.models, surveyResponse.id);
          ...
          const surveyResponses = await this.models.surveyResponse.findManyByColumn(
            'entity_id',
            allEntityIds,
        ...
        const questions = await findQuestionsInSurvey(this.models, currentSurvey.id);
