...
exports.down = function(db) {
  ...
  return db.runSql(`
    ...
    END IF;
    IF TG_OP = 'INSERT' OR TG_OP = 'UPDATE' THEN
      PERFORM pg_notify('change', json_build_object('change', json_build_object('record_type', TG_TABLE_NAME, 'record_id', NEW.id, 'type', 'update'), 'record', row_to_json(NEW))::text);
      RETURN NEW;
    END IF;
    IF TG_OP = 'DELETE' THEN
      PERFORM pg_notify('change', json_build_object('change', json_build_object('record_type', TG_TABLE_NAME, 'record_id', OLD.id, 'type', 'delete'), 'record', row_to_json(OLD))::text);
      RETURN OLD;
    END IF;
    END;
    $$;
