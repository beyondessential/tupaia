...
describe('Permissions checker for Importing Entities', async () => {
  describe('Sufficient permissions when importing entities', () => {
    const accessPolicy = new AccessPolicy(DEFAULT_POLICY);

    it('Should allow importing entities within 1 country if users have access to the country that the entities are within', async () => {
    ...
    it('Should allow importing entities within multiple country if users have access to all the countries that the entities are within', async () => {
      const entitiesByCountryName = {
        Kiribati: [
          {
            code: 'KI_1',
  ...
  describe('Insufficient permissions when importing entities', async () => {
    const accessPolicy = new AccessPolicy(DEFAULT_POLICY);

    it('Should not allow import entities within 1 country if users do not have access to the country that the entities are within', async () => {
      const entitiesByCountryName = {
        Laos: [
          {
            code: 'LA_1',
