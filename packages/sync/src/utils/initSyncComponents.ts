import { NON_SYNCING_TABLES, SYNC_TICK_FLAGS } from '../constants';
...
export async function initSyncComponents(driver: DatabaseDriver, isClient: boolean = false) {
  ...
  const initialValue = isClient ? SYNC_TICK_FLAGS.LAST_UPDATED_ELSEWHERE : 0; // -999 on client, 0 on central server
