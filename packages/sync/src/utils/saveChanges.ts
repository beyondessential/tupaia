...
export const saveCreates = async (
  ...
  for (let i = 0; i < records.length; i += batchSize) {
    ...
    try {
      ...
      throw new Error(`Batch insert failed for model ${model.databaseRecord}: ${error.message}`);
    }
...
export const saveUpdates = async (
  ...
  for (let i = 0; i < recordsToSave.length; i += batchSize) {
    ...
    try {
      ...
      throw new Error(`Batch update failed for model ${model.databaseRecord}: ${error.message}`);
    }
...
export const saveDeletes = async (
  ...
  for (let i = 0; i < recordsForDelete.length; i += batchSize) {
    ...
    try {
      ...
      throw new Error(`Batch delete failed for model ${model.databaseRecord}: ${error.message}`);
    }
