...
describe('addRecentEntity', () => {
  ...
  it('Adds an entry to the recent entities list', async () => {
    ...
    expect(mockUser).toMatchObject({ preferences: { recentEntities: { DL: { district: ['DL_1'] } } } })
  });
  ...
  it('Adds multiple entries to the recent entities list if it already exists in the list', async () => {
    ...
    expect(mockUser).toMatchObject({ preferences: { recentEntities: { DL: { district: ['DL_3', 'DL_2', 'DL_1'] } } } })
  });
  it('Cycles out the last entry when exceeded MAX = 3', async () => {
    ...
    expect(mockUser).toMatchObject({ preferences: { recentEntities: { DL: { district: ['DL_5', 'DL_4', 'DL_3'] } } } })
  });
  it('Separately stores lists for different entity types', async () => {
    ...
    expect(mockUser).toMatchObject({ preferences: { recentEntities: { DL: { district: ['DL_3', 'DL_2', 'DL_1'], facility: ['DL_8', 'DL_7', 'DL_6'] } } } })
  });
  it('Separately stores lists for different countries', async () => {
    ...
    expect(mockUser).toMatchObject({ preferences: { recentEntities: { DL: { district: ['DL_3', 'DL_2', 'DL_1'] }, FJ: { district: ['FJ_3', 'FJ_2', 'FJ_1'] } } } })
  });
});
